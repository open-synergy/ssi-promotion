...
class TestUiPromotionCodeOperatingUnit(HttpSavepointCase):
    """UI/UX tour test for the ``promotion_code`` delta work instruction.
    ...
    **Operating Unit** field is rendered on the ``promotion_code`` create
    form.
    """

    @classmethod
    def setUpClass(cls):
        """Prepare the IK Pre-Conditions of the delta promotion_code tour.
        ...
        * ``Access`` of the base Flow -- the same user is put in
          ``ssi_promotion.promotion_code_user_group``, the group the
          base IK names as actor.
        ...
        groups = cls.env.ref("ssi_promotion.promotion_code_user_group") + cls.env.ref(
    ...
    def test_create(self):
        """Run the delta create tour for ``promotion_code``.

        The tour is delta-only: it walks Flow 1 to Flow 2 of the base IK
        (``ssi_promotion/docs/promotion_code/01-create.md``) to reach the
        blank create form, then asserts the **Operating Unit** field the
        delta IK adds to it.
